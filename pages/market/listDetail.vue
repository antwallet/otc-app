	...
	<view class="page flex-col">
		<block v-if="!loading">
				...
				<view class="kuang " v-for="(item, index) in accountTradeList" :key="index">
					<view class="fl_ju">
						<view class="danhao fl_al">
							{{item.energyBuyId}}{{item.premiumBuyId}}
							<image src="https://packet-gift.oss-ap-southeast-1.aliyuncs.com/2024/antwalletbotApp/images/copy.png" @click="fuzhi(item.energyBuyId)" mode=""></image>
						</view>
						<view :class="item.status == 0?'status success':'status error'">
							{{item.status == 0?i18n.marketList.success:i18n.marketList.failed}}
						</view>
					</view>
					<view class="address fl_ju">
						<view class="" v-if="type == 0">
							{{i18n.marketList.address}}：{{item.hexAddress}}
						</view>

					</view>
					<view class="jiage fl_ju">
						<view class="">
							<text class="name">{{i18n.marketList.price}}：</text>
							<text style="color: #EB4B46;font-size: 24rpx;">$</text>
							<text class="qian">{{item.money}}</text>
							<text class="name">{{item.energyType}} {{i18n.marketList.transaction}}</text>
						</view>
						<view class="date">
			...
			<block v-if="accountTradeList.length == 0">
				<u-empty mode="history" icon="https://packet-gift.oss-ap-southeast-1.aliyuncs.com/2024/antwalletbotApp/zwxx.png">
				</u-empty>
			</block>
		</block>
