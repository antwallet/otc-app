<template>
	<view class="hr-grid-swiper-wrap">
		<swiper circular :indicator-dots="false" :autoplay="true" @change="onSwiperChange">
			<swiper-item v-for="(group, index) in gridGroupList" :key="index">
				<view class="swiper-item-inner">
					<view v-for="(item, idx) in group" :key="idx" :style="itemStyles[idx]" class="item"
						@click="handleItemClick(idx, item)">
						<slot :item="item" :gridWidth="itemStyles[idx].width" :gridHeight="itemStyles[idx].height" />
					</view>
				</view>
			</swiper-item>
		</swiper>
		<!-- 自定义指示条 -->
		<slot v-if="gridGroupList.length > 1" name="indicator-bar" :current="current" :total="gridGroupList.length">
			<view class="custom-indicator-bar" :style="{
          margin: `${indicatorBarMarginTop}px auto`,
          width: indicatorBarWidth + 'px',
          height: indicatorBarHeight + 'px',
          backgroundColor: indicatorBackground,
        }">
				<slot name="indicator-dot" :current="current" :total="gridGroupList.length">
