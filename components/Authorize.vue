...
<script>
	...
	export default {
		...
		methods: {
			...
			getLogoUrl() {
				let that = this;
				console.log('v20201120 getLogoUrl')
				if (Cache.has(LOGO_URL)) {
					console.log('v20201120 getLogoUrl 111', Cache.get(LOGO_URL))

					this.logoUrl = Cache.get(LOGO_URL);
					return;
				}
				getLogo().then(res => {
					//  2020年12月1日 找到原因 字段拼错了！！ logoUrl
					// this.logoUrl = res.data.logo_url
					that.$set(that, "logoUrl", res.data.logoUrl)
					console.log('v20201120 getLogoUrl 222', res.data.logoUrl, that.logoUrl)
